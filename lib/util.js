...
(function() {
  var FekitConfig, Reader, UrlConvert, Writer, async, child_process, cjson, coffee, exit, fs, fstream, mkdirp, ncp, request, rimraf, sty, syspath, tar, utilarray, utilasync, utilconfig, utilfile, utilhttp, utillogger, utilpath, utilproc, utilsys, vm, yaml, zlib, _, _closest, _closest_dir, _runCode;
  ...
  _closest_dir = function(p, finddirname) {
    var dir, file, files, _i, _len;
    if (p === "/" || (process.platform === "win32" && p.match(/^[a-zA-Z]:(\\|\/)?$/))) {
      return null;
    }
  ...
  exports.path = utilpath = {
    ...
    closest: function(path, findfilename, is_directory) {
      if (is_directory) {
  ...
  */
  ...
  exports.http = utilhttp = {
    ...
    put: function(url, filepath, cb) {
      utillogger.log("fekit " + (sty.red('http')) + " " + (sty.green('PUT')) + " " + url);
      return fs.createReadStream(filepath).pipe(request.put(url, function(err, res, body) {
        return cb(err, body);
      }));
    }
  };
