...
(function() {
  var FekitConfig, Reader, UrlConvert, Writer, async, child_process, cjson, coffee, exit, fs, fstream, md5, md5_cache, mkdirp, ncp, os, request, rimraf, shell, sty, syspath, tar, utilarray, utilasync, utilconfig, utilfile, utilhttp, utillogger, utilpath, utilproc, utilrsync, utilsys, vm, yaml, zlib, _, _closest, _closest_dir, _runCode, _watch;
  ...
  exports.path = utilpath = {
    ...
    is_root: function(path) {
      if (process.platform === 'win32') {
        return RegExp("[a-z]:" + syspath.sep, "i").test(path);
      } else {
    ...
    each_directory: function(path, cb, is_recursion, root) {
      var f, list, p, _f, _i, _len, _results;
      if (root == null) {
        root = "";
      }
        ...
        if (!is_recursion) {
          ...
          if (utilpath.is_normalize_dirname(f)) {
            if (!utilpath.is_directory(p)) {
              ...
              _results.push(utilpath.each_directory(p, cb, is_recursion, root));
            }
          } else {
            _results.push(void 0);
          }
  ...
  utilfile.md5 = function(filepath) {
    var buf;
    if (md5_cache[filepath]) {
      return md5_cache[filepath];
    }
  ...
  */
  ...
  exports.proc = utilproc = {
    ...
    checkEnvironment: function(checklist, cb) {
      var i, list, n;
      n = function(cmd) {
        return function(done) {
          return done(!shell.which(cmd) ? cmd : null);
        };
      };
      list = (function() {
        var _i, _len, _results;
      ...
      return async.series(list, function(err) {
        if (err) {
    ...
    hasCommand: function(cmd, cb) {
      return child_process.exec(cmd, function(error) {
        return cb(!!error);
      });
    },
  ...
  exports.rsync = utilrsync = function(opts, cb) {
    var args, _args;
    ...
    return child_process.exec("rsync " + args, function(err, stdout, stderr) {
      ...
      if (stdout) {
        utillogger.log(stdout);
      }
      if (stderr) {
        utillogger.error(stderr);
      }
      return cb && cb(err, stdout, stderr);
    });
  };
