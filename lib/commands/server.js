...
(function() {
  ...
  setupServer = function(options) {
    ...
    app = connect().use(connect.logger('tiny')).use(connect.query()).use(middleware.mock(options)).use(connect.bodyParser()).use(middleware.velocity(options)).use(middleware.fekit(options)).use(connect["static"](options.cwd, {
      hidden: true,
      redirect: true
