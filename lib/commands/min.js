...
(function() {
  ...
  process_directory = function(options) {
    var EXPORT_LIST_KEY, conf, script_global;
    script_global = {
      EXPORT_LIST: [],
      util: utils
    };
    ...
    conf.each_export_files(function(srcpath) {
      ...
      return EXPORT_LIST_KEY[srcpath] = iter;
    });
    ...
    return conf.each_export_files_async(function(srcpath, parents, seriesCallback) {
      ...
      _done = function(err, source) {
        var ast, dest, final_code, md5code, _ref, _ref1;
        if (err) {
        ...
        if ((_ref = EXPORT_LIST_KEY[srcpath]) != null) {
          _ref.ver = md5code;
        }
        if ((_ref1 = EXPORT_LIST_KEY[srcpath]) != null) {
