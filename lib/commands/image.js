...
(function() {
  ...
  exports.run = function(options) {
    return utils.proc.checkEnvironment(['rsync', 'svn'], function() {
      var command, conf, _ref, _ref1, _ref2, _ref3;
      ...
      if (!utils.path.exists(options.images_path)) {
        err("项目中必须存在 images 目录才能使用该功能");
      }
      conf = utils.config.parse(options.cwd);
      if (!((_ref = conf.root) != null ? (_ref1 = _ref.export_global_config) != null ? _ref1.resource_domain : void 0 : void 0)) {
        err("fekit.config 中必须存在 export_global_config.resource_domain 配置节");
      }
      if (!((_ref2 = conf.root) != null ? (_ref3 = _ref2.export_global_config) != null ? _ref3.resource_pid : void 0 : void 0)) {
        err("fekit.config 中必须存在 export_global_config.resource_pid 配置节");
      }
      command = options['_'][1];
