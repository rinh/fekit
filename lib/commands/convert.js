...
(function() {
  var CONFIG, CSS_REG, CURR, FILE, JS_REG, LINK_CSS_REG, LINK_JS_REG, check, check_config, findit, get_config, process_app_css, process_app_javascript, process_build, process_config, process_srclist, process_ver, show_config, spawn, sysfs, syspath, utils, _each_files, _get_verpath, _replaceCSS, _replaceJS, _replaceSrclist;
  ...
  */
  ...
  _each_files = function(cb) {
    var config, ext, file, files, _i, _len, _results;
    config = get_config();
    files = findit.sync(CURR);
    _results = [];
    for (_i = 0, _len = files.length; _i < _len; _i++) {
      ...
      if (!config.filter.length || (config.filter.length && ~config.filter.indexOf(ext))) {
        if (cb(file, config)) {
          _results.push(utils.logger.log("已处理 " + file));
        } else {
