...
(function() {
  var PARAM_CACHE, charset, compiler, dns, md5, mime_config, onlineServerIP, qs, request, sysfs, syspath, sysurl, toMD5, toPARAM, urlrouter, utils,
    _this = this;
  ...
  dns.resolve4('qunarzz.com', function(err, addresses) {
    if (!err) {
      return onlineServerIP = addresses[0];
    }
  });
    ...
    return fekitRouter = urlrouter(function(app) {
      return app.get(utils.UrlConvert.PRODUCTION_REGEX, function(req, res, next) {
        ...
        if (utils.path.exists(srcpath)) {
          ...
          if (options.opposite && onlineServerIP) {
            return request('http://' + onlineServerIP + url.pathname, function(error, response, body) {
              if (!error && response.statusCode === 200) {
                return res.end(body);
              }
            });
          } else {
            return res.end("alert('文件不存在 " + url.pathname + "');");
          }
        }
      });
    });
