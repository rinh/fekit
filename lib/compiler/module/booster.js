...
(function() {
  var CHECKSUM_CACHED, COMPILED_CACHED, COMPILED_CACHED_DEPEND, MODULES, cached, fs, path, utils, watch;
  ...
  cached = function(filename) {
    try {
      return CHECKSUM_CACHED[filename] = utils.md5(utils.file.io.read(filename) + path.relative(process.cwd(), filename));
    } catch (err) {
