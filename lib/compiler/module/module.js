  ...
  */
  ...
  Module = (function() {
    ...
    Module.prototype.getCompileType = function() {
      var fkconf, fkconf_path, type;
      type = this.config.getCompileType();
      if (type === MODULE_COMPILE_TYPE.COMPONENT) {
        fkconf_path = utils.path.closest(this.path.getFullPath(), 'fekit.config', false, function(path) {
          var config;
          config = utils.file.io.readJSON(path);
          return config.compiler !== 'component';
        });
        if (!fkconf_path) {
          return MODULE_COMPILE_TYPE.UNKNOWN;
        }
    ...
    Module.prototype._process = function(path, cb) {
      ...
      if (plugin) {
        return plugin.process(this.source, path, function(err, result) {
          if (err) {
