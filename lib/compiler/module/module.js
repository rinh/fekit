...
(function() {
  var CSSModule, JSModule, MODULE_COMPILE_TYPE, MODULE_CONTENT_TYPE, Module, ModuleConfig, ModulePath, events, md5, parser, syspath, util, utils, _,
  ...
  */
  ...
  Module = (function() {
    ...
    Module.prototype._doMacro = function(source, config) {
      var reg;
      config = config || {};
      reg = /\/\*\[([^\]]+?)\]\*\//ig;
    ...
    Module.prototype._process = function(path, cb) {
      var ext, plugin, source;
      ext = syspath.extname(path);
      plugin = ModulePath.getPlugin(ext);
      if (plugin) {
        source = utils.removeBOM(this.source);
        try {
          source = this._doMacro(source, this.config.config.getEnvironmentConfig()[this.options.environment]);
        } catch (err) {
          utils.logger.error("在 environment 配置中找不到 " + this.options.environment + " 项");
          source = source;
        }
